...
import { Plus, Search, Edit2, Trash2, Download, TrendingDown } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">Track and categorize all business spending.</p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="bg-rose-500 text-white px-4 py-2 rounded-xl flex items-center hover:bg-rose-600 transition-colors shadow-lg shadow-rose-500/20"
        >
      ...
      <div className="flex gap-4">
        <div className="flex-1 relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/20 outline-none"
            value={searchTerm}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase">Paid To</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={record.id} className="hover:bg-slate-50/50 group">
                  <td className="px-6 py-4 font-medium text-slate-900">{record.paidTo}</td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-rose-50 text-rose-600 text-xs font-bold rounded-lg uppercase">
                      {record.category}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-bold text-rose-600">{formatCurrency(record.amount)}</td>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">Paid To</label>
                  <input required type="text" className="w-full px-4 py-2 border rounded-xl" value={formData.paidTo} onChange={e => setFormData({...formData, paidTo: e.target.value})} />
                </div>
                <div className="space-y-1">
              ...
              <button type="submit" className="w-full py-3 bg-rose-500 text-white font-bold rounded-xl shadow-lg shadow-rose-500/20">Save Record</button>
            </form>
          </motion.div>
        </div>
      )}
